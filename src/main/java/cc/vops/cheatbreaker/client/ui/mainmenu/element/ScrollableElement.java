...
public class ScrollableElement extends AbstractElement {
    ...
    @Override
    public void handleScroll(GuiGraphicsExtractor gui, int delta) {
        if (delta != 0 && this.scrollAmount > this.height) {
            this.internalScrollAmount += (float) delta * 50.0f;
        }
    }
    ...
    public void handleElementDraw(GuiGraphicsExtractor gfx, float f, float f2, boolean bl) {
        ...
        if (bl2) {
            ...
            RenderUtil.drawRect(gfx, this.x, this.y - f8, this.x + this.width, this.y + f7 - f8, bl4 || this.hovered ? -52429 : -4180940);
        }
    ...
    public void drawScrollable(GuiGraphicsExtractor gfx, float f, float f2, boolean bl) {
        ...
        gfx.pose().pushMatrix();
        gfx.pose().translate(0.0f, this.translateY);
    }

    public void scrollableOnMouseClick(GuiGraphicsExtractor gfx, float mouseX, float mouseY, boolean bl) {
        float f3;
        this.alwaysTrueApparently = true;
//        GL11.glPopMatrix();
        gfx.pose().popMatrix();
...
//            }
            ...
            RenderUtil.drawRect(gfx, this.x, this.y, this.x + this.width, this.y + this.height, -13158601);
            RenderUtil.drawRect(gfx, this.x, this.y + this.height + f8, this.x + this.width, this.y + this.height - f7 + f8, bl4 || this.hovered ? -52429 : -4180940);
        }
        if (!bl2 && this.translateY != 0.0f) {
            this.translateY = 0.0f;
        }
    ...
    public void handleScrollableMouseClicked(GuiGraphicsExtractor gfx, float f, float f2, boolean bl) {
        if (bl && (this.parent == null || this.parent.isMouseInside(f, f2)) && this.internalScrollAmount != 0.0) {
            this.translateY = (float) ((double) this.translateY - this.internalScrollAmount / (double) 8);
            this.internalScrollAmount = 0.0;
        }
